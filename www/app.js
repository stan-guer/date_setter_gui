/// <reference path="./vendor/jszip.d.ts" />
...
(() => {
	...
	const els = {
		...
		setDateButton: document.getElementById("setDateButton"),
	...
	function wireUi() {
		...
		els.applyAllButton.addEventListener("click", applyToAll);
	...
	async function loadFiles(fileList) {
		...
		if (!toRead.length) {
			showMessage(
				"No JPEG images found. Only .jpg/.jpeg files are supported.",
				"error",
			);
			return;
		}
		...
		const results = await readWithConcurrency(toRead, 8, async (file) => {
			const item = await readOne(file);
			done += 1;
			showMessage(`Loading images… ${done}/${toRead.length}`, "");
			return item;
		});
		...
		for (let i = 0; i < results.length; i += 1) {
			...
			if (item) {
				...
				skippedNames.push(toRead[i].name);
			}
		}
		...
		if (skipped) {
			summary += ` Skipped ${skipped} unsupported or unreadable file${skipped === 1 ? "" : "s"}.`;
		}
		showMessage(summary, skipped ? "warning" : "ok");
	}
	...
	async function readWithConcurrency(items, limit, worker) {
		const results = [];
		let next = 0;
	...
	function renderItemPanel() {
		...
		renderFileFacts(
			item
				? { name: item.name, size: item.buffer.byteLength, type: item.type }
				: null,
		);
	...
	function renderQueue() {
		...
		for (let i = 0; i < state.items.length; i += 1) {
			...
			badge.textContent = item.pendingDate ? "edited" : "untouched";
		...
		const selected = els.queueList.children[state.currentIndex];
		if (selected) {
	...
	function updateQueueSelection() {
		const children = els.queueList.children;
		for (let i = 0; i < children.length; i += 1) {
			children[i].classList.toggle("is-selected", i === state.currentIndex);
		}
	}
	...
	function setDate() {
		...
		if (!exifDate) {
			showMessage("Choose a valid date and time before setting.", "error");
			return;
		}

		item.pendingDate = exifDate;
		renderQueue();
		updateControls();

		const wasLast = state.currentIndex >= state.items.length - 1;
		if (wasLast) {
			showMessage(
				`Date set for "${item.name}". This was the last image.`,
				"ok",
			);
		} else {
			showMessage(`Date set for "${item.name}".`, "ok");
			selectItem(state.currentIndex + 1);
		}
	}
	...
	function applyToAll() {
		const exifDate = buildExifDateFromInputs();
		if (!exifDate) {
			showMessage("Choose a valid date and time first.", "error");
			return;
		}
		...
		updateControls();
		showMessage(
			`Applied the date to all ${state.items.length} image${state.items.length === 1 ? "" : "s"}.`,
			"ok",
		);
	}

	async function downloadEdited() {
		const edited = state.items.filter((item) => item.pendingDate);
		if (!edited.length) {
			return;
		}

		const downloadButton = els.downloadButton;
		const originalText = downloadButton.textContent;
		downloadButton.disabled = true;
		downloadButton.textContent = "Packaging…";

		try {
			// JSZip is a UMD global provided by ./vendor/jszip.min.js.
			// @ts-expect-error - global not visible to the TypeScript checker
			const zip = new JSZip();
			const usedNames = new Set();
			let warningCount = 0;

			for (const item of edited) {
				const result = writeExifDates(item.buffer, item.pendingDate);
				warningCount += result.warnings.length;
				const blob = new Blob([result.buffer], {
					type: item.type || "image/jpeg",
				});
				zip.file(uniqueEditedFilename(item.name, usedNames), blob);
			}
			...
			triggerDownload(zipBlob, "exif-date-edited.zip");

			let suffix = "";
			if (warningCount) {
				suffix = ` ${warningCount} note${warningCount === 1 ? "" : "s"} about EXIF blocks.`;
			}
			showMessage(
				`Packaged ${edited.length} edited image${edited.length === 1 ? "" : "s"} as exif-date-edited.zip.${suffix}`,
				warningCount ? "warning" : "ok",
			);
		} catch (error) {
			showMessage(
				error.message || "The edited images could not be packaged.",
				"error",
			);
		} finally {
	...
	function uniqueEditedFilename(name, usedNames) {
		const base = editedFilename(name);
	...
	function resetUi() {
		...
		updateControls();
		showMessage(
			"Choose or drop JPEGs to start. Add more anytime — they're appended to the queue.",
			"",
		);
	}
	...
	function updateControls() {
		...
		const editedCount = state.items.filter((item) => item.pendingDate).length;
		const validDate = Boolean(buildExifDateFromInputs());

		enableInputs(hasItems);
		els.setDateButton.disabled = !hasItems || !validDate;
		els.resetButton.disabled = !hasItems;
		els.applyAllButton.disabled = !hasItems || !validDate;
		els.prevButton.disabled = state.currentIndex <= 0;
		els.nextButton.disabled = state.currentIndex >= state.items.length - 1;
		els.downloadButton.disabled = editedCount === 0;
		els.downloadButton.textContent = `Download edited (${editedCount})`;
		els.counter.textContent = hasItems
	...
	function renderStats() {
		...
		const edited = state.items.filter((item) => item.pendingDate).length;
		const totalBytes = state.items.reduce(
			(sum, item) => sum + item.buffer.byteLength,
			0,
		);
		els.stats.textContent = `${loaded} loaded · ${edited} edited · ${formatBytes(totalBytes)}`;
	}
	...
	function seedInputs(item) {
		...
		const exifDate = item.pendingDate || preferredOriginalDate(item.metadata);
		if (!exifDate) {
			els.dateInput.value = "";
			els.timeInput.value = "";
			els.secondsInput.value = "0";
		...
		setInputsFromExif(exifDate);
	}

	function setInputsFromExif(exifDate) {
		const parts = parseExifDateParts(exifDate);
		if (!parts) {
			return;
		}
	...
	function buildExifDateFromInputs() {
		...
		const secondsText =
			els.secondsInput.value === "" ? "0" : els.secondsInput.value;
	...
	function writeExifDates(buffer, exifDate) {
		if (!isExifDateString(exifDate)) {
			throw new Error(
				"The selected date cannot be converted to a valid EXIF date.",
			);
		}
	...
	function parseTiff(tiff) {
		...
		const exifPointer = getLongValue(
			findEntry(ifd0, TAGS.ExifIFDPointer),
			parsed,
		);
		if (exifPointer) {
		...
		const gpsPointer = getLongValue(
			findEntry(ifd0, TAGS.GPSIFDPointer),
			parsed,
		);
		if (gpsPointer) {
	...
	function parseIfdAt(parsed, name, offset, visited) {
		...
		for (let i = 0; i < count; i += 1) {
			...
			const dataOffset = inline
				? valueFieldOffset
				: readU32(tiff, valueFieldOffset, parsed.little);
			let valueBytes;

			if (inline) {
				valueBytes = tiff.slice(
					valueFieldOffset,
					valueFieldOffset + Math.min(byteCount, 4),
				);
				...
				parsed.warnings.push(
					`Tag 0x${tag.toString(16)} points outside the EXIF block.`,
				);
			}
	...
	function managedDateEntries(parsed) {
		...
		return {
			DateTimeOriginal: exifIfd
				? findEntry(exifIfd, TAGS.DateTimeOriginal)
				: null,
			DateTime: ifd0 ? findEntry(ifd0, TAGS.DateTime) : null,
			DateTimeDigitized: exifIfd
				? findEntry(exifIfd, TAGS.DateTimeDigitized)
				: null,
		};
	}
	...
	function shouldSkipForRebuild(ifdName, tag) {
		if (ifdName === "IFD0") {
			return (
				tag === TAGS.DateTime ||
				tag === TAGS.ExifIFDPointer ||
				tag === TAGS.GPSIFDPointer
			);
		}
		if (ifdName === "ExifIFD") {
			return (
	...
	function cloneEntry(entry) {
		if (entry.byteCount > 4 && entry.valueBytes.length !== entry.byteCount) {
			throw new Error(
				"The EXIF metadata contains truncated tag data and cannot be safely rebuilt.",
			);
		}
		return {
			tag: entry.tag,
	...
	function serializeTiff(ifds, little) {
		const order = ["IFD0", "ExifIFD", "GPS", "Interop", "IFD1"].filter((name) =>
			ifds.has(name),
		);
		...
		for (const name of order) {
			...
			const nextOffset =
				ifd.nextName && ifds.has(ifd.nextName)
					? ifds.get(ifd.nextName).offset
					: 0;
			writeU32(
				out,
				ifd.offset + 2 + ifd.entries.length * 12,
				nextOffset,
				little,
			);
		}
	...
	function buildExifApp1Segment(tiff) {
		...
		if (segmentLength > 0xffff) {
			throw new Error(
				"The edited EXIF metadata is too large for one JPEG APP1 segment.",
			);
		}
